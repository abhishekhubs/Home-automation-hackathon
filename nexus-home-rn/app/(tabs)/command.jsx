    ...
    <View style={styles.responseCard}>
      ...
      {response.reasoning && (
        <TouchableOpacity style={styles.whyRow} onPress={() => setShowReasoning(!showReasoning)}>
          <Text style={styles.whyText}>{showReasoning ? '▲' : '▼'} Why did decode do this?</Text>
        </TouchableOpacity>
      )}
...
export default function CommandScreen() {
  ...
  const handleSubmit = useCallback(async (text) => {
    ...
    try {
      ...
      const result = await callGroq(state.apiKey, buildSmartHomeSystemPrompt(state.rooms, time), cmd);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.inputBar}>
            <TextInput value={input} onChangeText={setInput} placeholder="Tell decode what to do..." placeholderTextColor="#94a3b8" style={styles.textInput} multiline returnKeyType="send" onSubmitEditing={() => handleSubmit()} />
          ...
          {isLoading && (
            <View style={styles.loadingCard}>
              ...
              <Text style={styles.loadingText}>decode is thinking…</Text>
            </View>
          )}
